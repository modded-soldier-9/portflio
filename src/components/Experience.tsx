      ...
      <div className="space-y-6">
        {experiences.map((exp) => (
          <article key={exp.id} className="relative pl-5 border-l border-rule">
            <span className="absolute left-[-3.5px] top-2 w-[6px] h-[6px] rounded-full bg-ink" aria-hidden="true" />
            ...
            <p className="text-sm text-ink-muted font-medium mb-1.5">{exp.company}</p>
            <p className="text-sm text-ink-muted leading-relaxed">{exp.description}</p>
            {exp.achievements.length > 0 && (
              <ul className="mt-2 space-y-0.5">
                {exp.achievements.map((a) => (
                  <li key={a} className="text-xs text-ink-muted before:content-['→_'] before:text-ink-faint">{a}</li>
                ))}
              </ul>
            )}
            {exp.skills.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {exp.skills.map((s) => (
                  <span key={s} className="px-2 py-0.5 text-[11px] rounded border border-rule text-ink-muted">
                    {s}
                  </span>
                ))}
              </div>
